...
export async function getBoard(boardId: string): Promise<(Board & { columns: BoardColumn[] }) | null> {
  const supabase = db();
  ...
  const cols: BoardColumn[] = (columns ?? []).map((col) => ({
    ...col,
...
export async function getBoardByProject(projectId: string): Promise<(Board & { columns: BoardColumn[] }) | null> {
  const supabase = db();
  const { data: board } = await supabase
    .from("boards")
    .select("*")
...
export async function getPublicBoard(token: string): Promise<(Board & { columns: BoardColumn[] }) | null> {
  const supabase = db();
  const { data: board } = await supabase
    .from("boards")
    .select("*")
...
export async function createBoard(title: string, projectId?: string): Promise<Board> {
  ...
  await Promise.all([
    supabase.from("board_columns").insert({ board_id: data.id, title: "Ideas", position: 0 }),
    supabase.from("board_columns").insert({ board_id: data.id, title: "In Development", position: 1 }),
    supabase.from("board_columns").insert({ board_id: data.id, title: "Ready", position: 2 }),
  ]);
...
export async function createCard(
  ...
  const { data, error } = await db()
    .from("board_cards")
    .insert({ board_id: boardId, column_id: columnId, type, content, position })
    .select()
    .single();
...
export async function pushShotToShotList(projectId: string, shotContent: Record<string, unknown>): Promise<void> {
  ...
  // Get or create a shot list for this project
