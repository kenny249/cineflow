...
export async function updateRetainerDeliverable(id: string, updates: { status?: string; title?: string; notes?: string; revision_notes?: string; revision_count?: number; revision_status?: string; brief_metadata?: Record<string, unknown>; sort_order?: number }): Promise<void> {
  const { error } = await db()
    .from('retainer_deliverables')
    .update(updates)
    .eq('id', id);
