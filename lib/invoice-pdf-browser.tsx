...
export function InvoicePdfDocumentBrowser({ invoice, profile, logoBase64 }: Props) {
  ...
  const s = StyleSheet.create({
    ...
    totDiscount: { fontSize: 10, color: GREEN },
    ...
    <Document>
      <Page size="LETTER" style={s.page}>
        ...
        <View style={s.header}>
          <View style={s.headerRow}>
            ...
            <View>
              ...
              {invoice.po_number ? <Text style={s.invMeta}>PO: {invoice.po_number}</Text> : null}
        ...
        <View style={s.billTo}>
          ...
          {invoice.client_address ? <Text style={s.billDetail}>{invoice.client_address}</Text> : null}
          {invoice.description ? (
            <Text style={[s.billDetail, { marginTop: 6, fontFamily: "Helvetica-Oblique" }]}>{invoice.description}</Text>
          ) : null}
        </View>
            ...
            <>
              ...
              {ps.ach_routing && ps.ach_account ? (
                <View style={s.payBlock}>
                  ...
                  {ps.ach_bank_name ? <View style={s.payRow}><Text style={s.payKey}>Bank</Text><Text style={s.payData}>{ps.ach_bank_name}</Text></View> : null}
              ...
              {(ps.check_payable_to || ps.check_mail_to) ? (
                <View style={s.payBlock}>
                  ...
                  {ps.check_payable_to ? <Text style={s.payVal}>Make payable to: {ps.check_payable_to}</Text> : null}
                  {ps.check_mail_to ? <Text style={[s.payVal, { marginTop: 4 }]}>Mail to: {ps.check_mail_to}</Text> : null}
                </View>
              ) : null}
            </>
          )}
